...
export async function init() {
    ...
    if (filteredPlaybooks.length === 0) {
        ...
        const { playbookId } = await inquirer.prompt([{
                ...
                choices: filteredPlaybooks.flatMap(p => p.paths.map((pathObj) => ({
                    name: `${pathObj.name} — ${pathObj.description}`,
                    value: pathObj.id
                })))
            }]);
        const activePlaybook = filteredPlaybooks.find(p => p.paths.some((pathObj) => pathObj.id === playbookId));
        const activePath = activePlaybook?.paths.find((pathObj) => pathObj.id === playbookId);
