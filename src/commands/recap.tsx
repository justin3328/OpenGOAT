...
export async function weeklyRecap() {
  ...
  for (const goal of goals) {
    const missions = MissionsRepo.getByGoal(goal.id, 1);
    const score = calculateScore(missions as any, 1);
    ...
    render(
      <Box flexDirection="column" borderStyle="double" borderColor="yellow" padding={1} marginY={1}>
        <Text bold inverse color="yellow"> WEEKLY RECAP: {goal.statement.toUpperCase()} </Text>
        <Box marginTop={1}>
