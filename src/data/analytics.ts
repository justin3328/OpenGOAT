...
export function velocityTrend(goalId: string, weeks: number = 8): number[] {
  const db = getDB();
  const rows = db.prepare('SELECT velocity FROM week_scores WHERE goal_id = ? ORDER BY week_number DESC LIMIT ?').all(goalId, weeks) as any[];
  return rows.map(r => r.velocity).reverse();
}
...
export function paceProjection(goalId: string): number | null {
  ...
  if (!goal) return null;
  const recentWeek: any = db.prepare('SELECT velocity FROM week_scores WHERE goal_id = ? ORDER BY week_number DESC LIMIT 1').get(goalId);
  if (!recentWeek || recentWeek.velocity === 0) return null;
  
  const gap = goal.target_val - goal.current_val;
  return Math.ceil(gap / recentWeek.velocity);
}
